...
export class MinecraftStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: MinecraftStackProps) {
    ...
    // DeleteOnTermination is set to false on the CfnVolumeAttachment below.
    ...
    const volumeAttachment = new ec2.CfnVolumeAttachment(this, 'MinecraftVolumeAttachment', {
      ...
      device: '/dev/sdf',
      // Note: on Nitro instances the Linux device is /dev/nvme*n1, not /dev/sdf.
      // The bootstrap script identifies the device by NVMe serial number (Volume ID),
      // not by this attachment name.
    });
